...
export default function Navbar() {
  ...
  return (
    <>
      <nav
        className={`fixed border-b border-transparent transition-colors duration-300 py-5 top-0 inset-x-0 z-50 ${
          ...
          <div className="flex justify-between w-full">
            {navLinks.map(({ href, label }) => (
              <Link href={href} key={label}>
                <a
                  className={`uppercase rounded-md px-2 md:px-6 py-2 font-semibold transition-colors duration-100 ${
                    router.pathname === href
                      ? "text-black bg-neutral-100 hover:text-black"
